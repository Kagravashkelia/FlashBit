...
int quiescence(chess::Board& board, int alpha, int beta) {
    ...
    for (size_t i = 0; i < num_moves; ++i) {
        ...
        // Delta Pruning subido de tono: ignoramos capturas que no mejoran materialmente
        if (move.typeOf() != chess::Move::PROMOTION && (stand_pat + victim_val + 250 < alpha)) {
            continue;
        }
...
// ============================================================================
// 5. BÚSQUEDA NEGAMAX PRINCIPAL (EXTREMADAMENTE SELECTIVA)
// ============================================================================
int negamax(chess::Board& board, int depth, int alpha, int beta, int ply) {
    ...
    // ========================================================================
    // PODAS PREVIAS AL MOVIMIENTO (MÁXIMA BRUTALIDAD)
    // ========================================================================
    if (!is_pv_node && !in_check) {
        
        // 1. RAZORING MÁS PROFUNDO (hasta depth 4)
        if (depth <= 4) {
        ...
        // 2. NULL MOVE PRUNING A ESTEROIDES
        // Ahora reducimos mucho más agresivo: (depth / 2). Corta medio árbol de golpe.
        if (depth >= 3 && eval >= beta && board.hasNonPawnMaterial(board.sideToMove()) && std::abs(eval) < MATE_SCORE - 1000) {
            board.makeNullMove();
        ...
        // 3. REVERSE FUTILITY PRUNING EXTENDIDO Y ESTRECHADO (hasta depth 7)
    ...
    // Futility pruning agresivo subido hasta profundidad 8
    ...
    for (size_t i = 0; i < num_moves; ++i) {
        ...
        if (is_quiet) {
            ...
            // HISTORY PRUNING (Nueva barrera de seguridad): 
            // Si la jugada apesta históricamente (score negativo) y estamos a baja prof, la ignoramos.
            if (!is_pv_node && !in_check && depth <= 4 && max_score < -1000) {
                continue; 
            }

            // LMP: Cortamos casi de inmediato basándonos en la nueva tabla ultra limitante
            if (!is_pv_node && !in_check && depth <= 8 && quiet_moves_searched > lmp_threshold) {
                continue; 
            }
        ...
        if (moves_searched == 1) {
            ...
            // LMR TRIGGER ANTICIPADO: Antes exigíamos depth >= 3 y move > 2. 
            // ¡Ahora reducimos casi TODO a partir del segundo movimiento si depth >= 2!
            bool can_lmr = (depth >= 2 && moves_searched > 1 && !in_check && !is_capture && !gives_check && !is_pv_node && !is_promotion);
            ...
            if (can_lmr) {
                ...
                // Penalizar jugadas silenciosas odiadas por el historial
                if (is_quiet && history_table[static_cast<int>(board.sideToMove()^1)][move.from().index()][move.to().index()] < 0) {
                    R++; // Aún más reducción
                }
                // Bonificar jugadas con gran historial
                else if (is_quiet && history_table[static_cast<int>(board.sideToMove()^1)][move.from().index()][move.to().index()] > 10000) {
                    R--;
                }
            ...
            if (is_quiet && ply < MAX_PLY) {
                ...
                for (size_t p = 0; p < i; ++p) {
                    ...
                    if (!board.isCapture(penalized_move) && penalized_move.typeOf() != chess::Move::PROMOTION) {
                        int& p_hist = history_table[color][penalized_move.from().index()][penalized_move.to().index()];
